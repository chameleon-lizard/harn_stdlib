...
def test_compaction_threshold_and_usage_helpers() -> None:
    ...
    assert (
        should_compact(95000, 100000, CompactionSettings(enabled=False, reserveTokens=1, keepRecentTokens=1))
        is False
    )
...
async def test_generate_summary_and_compact_behaviour(registrations: list[Any]) -> None:
    ...
    assert (
        "Test summary"
        in await generate_summary(messages, reasoning_model, 2000, "test-key", thinking_level="medium")
    )
    ...
    with pytest.raises(RuntimeError, match="Summarization failed: boom"):
        await generate_summary(messages, error_model, 2000, "test-key")

    clamp_model = create_faux_model(registrations, reasoning=False, max_tokens=128000)
    seen_max_tokens: list[dict[str, Any] | None] = []
    registrations[-1].set_responses(
        [
            capture_options_response(seen_max_tokens, "## Goal\nTest summary"),
            capture_options_response(seen_max_tokens, "## Goal\nTest summary"),
        ]
    )
    ...
    registrations[-1].set_responses([capture_options_response(prefix_options, "## Original Request\nTest summary")])
    await compact(
        CompactionPreparation(
            firstKeptEntryId="entry-keep",
            messagesToSummarize=[],
    ...
    with pytest.raises(RuntimeError, match="Turn prefix summarization failed: prefix failed"):
        await compact(
            CompactionPreparation(
                firstKeptEntryId="entry-keep",
                messagesToSummarize=[],
...
@pytest.mark.asyncio
async def test_branch_summary_collection_and_generation(registrations: list[Any], tmp_path) -> None:
    session = SessionManager.inMemory(str(tmp_path))
    root = session.appendMessage(create_user_message("root"))
    branch_user = session.appendMessage(create_user_message("branch user"))
    branch_assistant = session.appendMessage(
    ...
    session.branch(root)
    target = session.appendMessage(create_user_message("target"))

    collected = collect_entries_for_branch_summary(session, branch_assistant, target)
    ...
    assert branch_preparation.messages[0].role == "branchSummary"
    ...
    assert error_result.error == "branch boom"
