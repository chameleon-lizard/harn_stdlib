...
edit-diff traces, TUI session persistence, numbered `/continue` selection,
context status reporting, Ctrl+O raw input handling, Esc/Ctrl+C cancellation,
built-in system-prompt operating instructions, skill discovery and system-prompt
injection, streamed reasoning overlap deduplication, and streamed reasoning
