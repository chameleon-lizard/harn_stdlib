...
from harn_ai.models import models_are_equal
from harn_ai.types import ImageContent
from harn_tui import ProcessTerminal, TUI, setKeybindings

from harn_coding_agent.cli import session_picker
from harn_coding_agent.cli.args import Args, parse_args, print_help
from harn_coding_agent.cli.file_processor import ProcessFileOptions, process_file_arguments
...
from harn_coding_agent.config import ENV_SESSION_DIR, VERSION, expand_tilde_path, get_agent_dir, get_package_dir
...
from harn_coding_agent.core.auth_guidance import formatNoModelsAvailableMessage
from harn_coding_agent.core.auth_storage import AuthStorage
from harn_coding_agent.core.export_html import export_from_file
...
from harn_coding_agent.core.model_resolver import ScopedModel, resolveCliModel, resolveModelScope
from harn_coding_agent.core.output_guard import isStdoutTakenOver, restoreStdout, takeOverStdout
...
from harn_coding_agent.core.timings import print_timings, reset_timings, time as time_mark
from harn_coding_agent.migrations import run_migrations, show_deprecation_warnings
from harn_coding_agent.modes import runPrintMode as run_print_mode
from harn_coding_agent.modes.interactive.components.extension_selector import ExtensionSelectorComponent
from harn_coding_agent.modes.interactive import InteractiveMode
from harn_coding_agent.modes.interactive.theme.theme import init_theme, stop_theme_watcher
from harn_coding_agent.modes.rpc import run_rpc_mode
from harn_coding_agent.package_manager_cli import (
    _take_command_exit_code,
    handle_config_command,
    handle_package_command,
)
from harn_coding_agent.utils.paths import is_local_path, normalize_path, resolve_path
from harn_coding_agent.utils.windows_self_update import cleanup_windows_self_update_quarantine
...
async def main(args: list[str], options: MainOptions | None = None) -> int:
    reset_timings()
    if "--offline" in args or is_truthy_env_flag(os.environ.get("HARN_OFFLINE")):
        os.environ["HARN_OFFLINE"] = "1"
        os.environ["HARN_SKIP_VERSION_CHECK"] = "1"
    ...
    try:
        ...
        startup_benchmark = is_truthy_env_flag(os.environ.get("HARN_STARTUP_BENCHMARK"))
        if startup_benchmark and app_mode != "interactive":
            print(
                _format_colored_message("Error: HARN_STARTUP_BENCHMARK only supports interactive mode", _RED),
                file=sys.stderr,
            )
            return 1
